...
from apps.notifications.task import send_daily_notification
...
def test_send_daily_notification_sends_message(
    ...
    mocker.patch(
        "apps.notifications.task.YNABClient.get_categories_by_id",
        return_value=[category_grocery, category_dining_out],
    )
    mock_send = mocker.patch(
        "apps.notifications.task.DiscordService.send",
        return_value=True,
    )
    mock_format = mocker.patch(
        "apps.notifications.task.DiscordService.format_message",
